...
export default function GlobalError({
  ...
  return (
    <html>
      <body className="bg-black text-zinc-300 font-mono select-none">
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#080b11] px-6 retro-grid-bg">
          ...
          >
            ...
            <div className="absolute top-2 left-2 text-[10px] text-red-800 font-bold font-mono">┌ [ERR_GLOBAL]</div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 bg-black/40 border border-red-950/40 p-4 mb-6 text-[10px] md:text-xs text-red-700/80 font-mono">
              <div>[SECTOR] : <span className="text-red-500">0xGLOBAL</span></div>
              <div>[STATUS] : <span className="text-red-600 font-bold">ROOT_CRASH</span></div>
              <div>[THREAD] : <span className="text-zinc-500">ROOT_THREAD</span></div>
            ...
            <div className="space-y-4 mb-8 text-xs md:text-sm">
              <p className="text-zinc-400">A global application crash occurred in the root layout.</p>
              <div className="bg-red-950/20 border border-red-900/30 p-3 text-red-400 font-mono text-[11px] break-all max-h-24 overflow-y-auto">
                {error.message || "Global Render Exception"}
              </div>
            </div>
